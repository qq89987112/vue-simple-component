<template>
  <Upload action="//jsonplaceholder.typicode.com/posts/" class="upload-component" :show-upload-list="false" :on-success="onUploaded">
    <div v-if="!$slots.default" class="default-upload">
      <img v-if="value" :src="value"  class="preview-image" >
      <span v-else>+</span>
    </div>
    <slot></slot>
  </Upload>
</template>
<script>
    export default {
        name: 'upload',
        props:["value"],
        methods:{
          onUploaded(response, file, fileList){
            this.$emit("input",'data:image/png;base64,');
            this.$emit("uploaded",'data:image/png;base64,');
          }
        }
    }
</script>
<style lang="scss" rel="stylesheet/scss" scoped>
  .upload-component{
    .default-upload{
      width: 80px;
      line-height: 80px;
      border: 2px dashed #e4e4e4;
      text-align: center;
      font-size: 20px;

      .preview-image{
        width: 100%;
        height: 100%;
      }
    }
  }
</style>
